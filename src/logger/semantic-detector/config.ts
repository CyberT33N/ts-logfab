...
import { EOperationType, EDomainType, type IPatternConfig } from './types.ts'
...
export const DEFAULT_PATTERN_CONFIG = {
    ...
    domains: {
        [EDomainType.user]: [
            'user', 'account', 'profile', 'customer', 'person', 'member', 'client', 'employee'
        ] as const,
        [EDomainType.order]: [
            'order', 'purchase', 'transaction', 'cart', 'checkout', 'booking', 'reservation'
        ] as const,
        [EDomainType.product]: [
            'product', 'item', 'catalog', 'inventory', 'stock', 'goods', 'article', 'sku'
        ] as const,
        [EDomainType.finance]: [
            'payment', 'billing', 'invoice', 'tax', 'price', 'cost', 'money', 'currency', 'wallet'
        ] as const,
        [EDomainType.auth]: [
            'auth', 'login', 'logout', 'token', 'session', 'permission', 'role', 'access', 'security'
        ] as const,
        [EDomainType.notification]: [
            'notification', 'message', 'email', 'sms', 'alert', 'reminder', 'mail'
        ] as const,
        [EDomainType.analytics]: [
            'analytics', 'metrics', 'stats', 'report', 'tracking', 'event', 'log', 'monitor'
        ] as const,
        [EDomainType.integration]: [
            'api', 'webhook', 'sync', 'import', 'export', 'integration', 'external'
        ] as const,
        [EDomainType.system]: [
            'system', 'config', 'setting', 'admin', 'health', 'status', 'info', 'debug'
        ] as const,
        [EDomainType.general]: [] as const
    },
    complexityIndicators: {
        high: ['batch', 'bulk', 'mass', 'complex', 'advanced', 'deep', 'recursive', 'heavy'] as const,
        medium: ['multi', 'group', 'collection', 'list', 'array', 'set', 'range'] as const
    },
    costIndicators: {
        expensive: ['migrate', 'rebuild', 'reindex', 'backup', 'restore', 'export', 'import'] as const,
        high: ['batch', 'bulk', 'mass', 'analyze', 'aggregate', 'report', 'calculate'] as const,
        medium: ['search', 'query', 'process', 'transform', 'validate', 'sync'] as const
    }
